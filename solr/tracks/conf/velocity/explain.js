...
function formatScore(score, debugscore) {
  ...
  sQstr +=
    '<div class="score-field font80">Contributed by Fields = ' +
    scoreJson.totalContributedFieldScore +
    '</div>';
  sQstr +=
    '<div class="score-field font80">Contributed by Boosts = ' +
    scoreJson.boost.contributedScore +
    '</div>';
  ...
  sQstr += '</div>';
  sQstr +=
    '<div><a href="#" onclick="jQuery(this).siblings(\'pre\').toggle(); return false;">Show Raw Explain Data</a>';
  sQstr +=
    '<pre class="score-field font80" style="display:none;">' +
    JSON.stringify(debugscore, undefined, 2) +
    '</pre></div>';
...
function buildBoostGraph(obj, title, total) {
  ...
  sQstr +=
    '<div class="score-field">Score = ' +
    obj.contributedScore +
...
function buildScoreGraph(arr, title) {
  ...
  for (var i = 0; i < arr.length; i++) {
    if (parseFloat(arr[i].percentOfTotal) == 0.0) continue;
    var term = (field = '');
    ...
    sQstr +=
      '<div class="score-field" title="Score=' +
      arr[i].naturalScore +
      '">' +
      field +
      term +
      ' - <span class="score-percent">' +
      parseFloat(arr[i].percentOfTotal).toFixed(2) +
      '%</span></div>';
