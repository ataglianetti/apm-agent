...
ExplainParser.prototype = {
	...
	_init: function(explainStructure, roundTo) {
		...
		if (/^sum\sof:$/.test(explainStructure.description)
				&& /^FunctionQuery/.test(explainStructure.details[explainStructure.details.length - 1].description)) {
			this.out.boost.type = 'bf';
			this.out.boost.naturalScore = explainStructure.details[explainStructure.details.length - 1].details[0].value;
			this.out.boost.contributedScore = explainStructure.details[explainStructure.details.length - 1].value;
			this.out.boost.percentOfTotal = this.out.boost.contributedScore / this.out.total * 100;
			this.out.boost.formula = explainStructure.details[explainStructure.details.length - 1].description;
		} else if (/^boost.+?product\sof:$/.test(explainStructure.description)) {
			this.out.boost.type = 'boost';
			this.out.boost.naturalScore = explainStructure.details[explainStructure.details.length - 1].value;
		...
		for (var x = 0; x < this.out.byFieldTerm.length; x++) {
			...
			fields[this.out.byFieldTerm[x].field].contributedScore += this.out.byFieldTerm[x].contributedScore;
			terms[this.out.byFieldTerm[x].term].naturalScore += this.out.byFieldTerm[x].naturalScore;
			terms[this.out.byFieldTerm[x].term].contributedScore += this.out.byFieldTerm[x].contributedScore;
		}
	...
	_recurseFindFieldTerms: function(node) {
		if (/^\*:\*, product of:$|^(\w+):\*, product of:$|^weight\((\w+):"(.+?)"|^weight\(([\w\*]+):([^\s\^]+)/.test(node.description)) {
			var match = /^\*:\*|^(\w+):\*|^weight\((\w+):"(.+?)"|^weight\(([\w\*]+):([^\s\^]+)/.exec(node.description);
			if (match) {
				if (match[1]) {
					return [{
						field: match[1],
	...
	getResults: function() {
		...
		if (boostObj.type != 'none') {
			...
			boostObj.contributedScore = Math.round(boostObj.contributedScore * this.roundTo) / this.roundTo;
		}
		
		return {
			total: Math.round(this.out.total * this.roundTo) / this.roundTo,
			totalContributedFieldScore: Math.round(this.out.totalContributedFieldScore * this.roundTo) / this.roundTo,
